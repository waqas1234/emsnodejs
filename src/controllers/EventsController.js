...
exports.adddlogs = async (req, res) => {
  try {
    var followedcompany = await FollowedCompanies.find({
      companyid: req.body.userid,
    });

    const result = await Promise.all(
      followedcompany.map(async (element) => {
  ...
  try {
    ...
    const result = await Promise.all(
      logs.map(async (element) => {
        var findCompany = await Company.findOne({
          _id: element.companyid,
        });
        if (findCompany) {
          return { ...element, company: findCompany.name };
        } else {
          return { ...element, findCompany: null };
        }
      })
    );

    const logslist = result.map((element) => {
      return {
        ...element._doc,
